...
extension DatabaseManager{
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                ref.setValue(userNode, withCompletionBlock: { [weak self] error, _ in
                    if let error = error {
                        print("Failed to update user conversations: \(error)")
                        completion(false)
                        return
                    }
    ...
    private func finishCreatingConversation(name: String, conversationID: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        guard let myEmail = UserDefaults.standard.value(forKey: "email") as? String else {
            print("Failed to retrieve sender email from UserDefaults")
            completion(false)
            return
        }
        ...
        print("Attempting to write message to Firebase: \(collectionMessage)")

        database.child("\(conversationID)").setValue(value, withCompletionBlock: { error, _ in
            if let error = error {
                print("Failed to write message to Firebase: \(error.localizedDescription)")
                completion(false)
                return
            }
            print("Message written successfully to Firebase")
            completion(true)
        })
    }
    ...
    /// Gets all mmessages for a given conversatinon
    ...
    // sends a message iwht target conversation and message
